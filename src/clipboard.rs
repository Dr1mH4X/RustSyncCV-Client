...
use crate::messages::{ClipboardUpdate, ClipboardUpdatePayload, MSG_TYPE_CLIPBOARD_UPDATE, CONTENT_TYPE_TEXT, CONTENT_TYPE_IMAGE_PNG};
// (WebSocket Message alias removed; no direct send here now)
use std::io::Cursor;
use std::sync::{Arc, atomic::{AtomicBool, Ordering}};
use tokio::sync::{broadcast, mpsc};
use base64::Engine; // for .encode on STANDARD engine
...
pub async fn start_clipboard_monitor(
    ...
    loop {
        ...
        let result = task::spawn_blocking(|| {
            ...
            // 优先尝试文本
            if let Ok(t) = cb.get_text() { return Ok::<_, String>(ClipboardContent::Text(t)); }
            // 再尝试图片
            if let Ok(img) = cb.get_image() { return Ok(ClipboardContent::Image(img)); }
                ...
                match content {
                    ...
                    ClipboardContent::Image(raw_img) => {
                        ...
                        if let Some(rgba) = RgbaImage::from_raw(
                            ...
                            if DynamicImage::ImageRgba8(rgba).write_to(&mut cursor, ImageFormat::Png).is_ok() {
                                let bytes = cursor.into_inner();
                                // 大小限制检查
                                if (bytes.len() as u64) > max_image_kb * 1024 {
                                    println!("[clip:image] skip oversized size={} limit={}KB", bytes.len(), max_image_kb);
                                } else {
                                // 计算简单哈希避免重复
                                ...
                                if Some(h) != last_image_hash {
                                    ...
                                    last_send_time = now;
                                    println!("[clip:image] broadcasting size={} bytes", bytes.len());
    ...
    while let Some(payload) = rx.recv().await {
        if payload.content_type == CONTENT_TYPE_TEXT { 
            ...
            let set_res = task::spawn_blocking(move || {
                let mut cb = Clipboard::new().map_err(|e| format!("Clipboard init error: {e}"))?;
                cb.set_text(text).map_err(|e| format!("Clipboard set_text error: {e}"))?;
                Ok::<(), String>(())
            }).await;
            if let Err(e) = set_res { eprintln!("[setter] join error: {e}"); }
            ...
            if let Ok(bytes) = base64::engine::general_purpose::STANDARD.decode(&payload.data) {
                ...
                if let Ok(img) = image::load_from_memory(&bytes) {
                    ...
                    let res = task::spawn_blocking(move || {
                        let mut cb = Clipboard::new().map_err(|e| format!("Clipboard init error: {e}"))?;
                        let data = arboard::ImageData { width: w as usize, height: h as usize, bytes: std::borrow::Cow::Owned(rgba.into_raw()) };
                        cb.set_image(data).map_err(|e| format!("Clipboard set_image error: {e}"))?;
                        Ok::<(), String>(())
                    }).await;
                    if let Err(e) = res { eprintln!("[setter] image join error: {e}"); }
                    disable_flag.store(false, Ordering::SeqCst);
                }
            }
        }
